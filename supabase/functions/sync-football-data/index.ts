...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!token || !supabaseUrl || !serviceRoleKey) {
      return json({ message: 'Missing Supabase or football-data.org secret.' }, 500);
    }
    ...
    if (settings?.api_last_sync) {
      ...
      if (Date.now() - lastSync < 90_000) {
        return json({ message: settings.api_last_message || '최근 동기화됨', skipped: true });
      }
    }

    const season = Deno.env.get('FOOTBALL_DATA_SEASON') || '2026';
    const response = await fetch(`https://api.football-data.org/v4/competitions/WC/matches?season=${season}`, {
      headers: { 'X-Auth-Token': token },
    });

    if (!response.ok) {
      const text = await response.text();
      return json({ message: `football-data.org error ${response.status}: ${text}` }, 502);
    }
    ...
    const { data: localMatches, error: matchError } = await supabase
      ...
      .select('id, stage, team_a, team_b, external_id, synced_pre, result_team, kickoff')
      .order('id');
    ...
    if (updated > 0) {
      await supabase.rpc('worldcup_touch');
    }
...
function findRemoteMatch(local: LocalMatch, remoteMatches: FootballDataMatch[]) {
  if (local.external_id) {
    return remoteMatches.find((match) => String(match.id) === String(local.external_id));
  }

  const localA = normalizeTeam(local.team_a);
  const localB = normalizeTeam(local.team_b);
  const teamMatch = remoteMatches.find((match) => {
    const home = [match.homeTeam.name, match.homeTeam.shortName, match.homeTeam.tla].map(normalizeTeam);
    const away = [match.awayTeam.name, match.awayTeam.shortName, match.awayTeam.tla].map(normalizeTeam);
...
function buildPatch(local: LocalMatch, match: FootballDataMatch) {
  ...
  if (local.synced_pre === false && hasRealTeams) {
...
function buildResultPatch(match: FootballDataMatch) {
  const winner =
    match.score.winner === 'HOME_TEAM' ? 'A' :
    match.score.winner === 'AWAY_TEAM' ? 'B' : null;
  ...
  const pen = duration === 'PENALTY_SHOOTOUT' ? (match.score.penalties || {}) : null;

  let resultRegular: 'win' | 'draw';
  if (duration === 'EXTRA_TIME' || duration === 'PENALTY_SHOOTOUT') {
    // the match was level after 90 minutes
    resultRegular = 'draw';
  } else {
    resultRegular = Number(ft.home ?? 0) === Number(ft.away ?? 0) ? 'draw' : 'win';
  }

  let team = winner;
  if (!team) {
    const h = Number(ft.home ?? 0), a = Number(ft.away ?? 0);
    team = h > a ? 'A' : a > h ? 'B' : null;
  }
  ...
  return {
    ...
    result_home: home,
    result_away: away,
    result_pen_home: pen ? (pen.home ?? null) : null,
    result_pen_away: pen ? (pen.away ?? null) : null,
